...
/**
 * @struct osc_square_state_t
 * @brief Structure interne pour stocker l'état de l'oscillateur carré
 */
typedef struct {
...
/**
 * @private
 * @fn static void osc_square_generate(osc_t *self, double freq, double sample_rate, double *buffer, size_t size)
 * @brief Génère un signal carré dans le buffer
 * @param self La structure de l'oscillateur
 * @param freq La fréquence en Hz
 * @param sampleRate Le taux d'échantillonnage en Hz
 */
static void osc_square_generate(osc_t *self, double freq, double sampleRate, double *buffer, size_t size) {
	osc_square_state_t *state = (osc_square_state_t *)self->state;
    double phaseIncrement = (2.0 * M_PI * freq) / sampleRate;

    for (size_t i = 0; i < size; i++) {
        buffer[i] = (state->currentPhase < M_PI) ? 1.0 : -1.0;
...
static void osc_square_destroy(osc_t *self) {
    if(self) {
        if(self->state) {
            free(self->state);
        }
...
osc_t *osc_square_create() {
    ...
    osc_square_state_t *state = malloc(sizeof(osc_square_state_t));
    
    state->currentPhase = 0.0;
    
    osc->state = (void *)state;
    osc->generate = osc_square_generate;
    osc->destroy = osc_square_destroy;
